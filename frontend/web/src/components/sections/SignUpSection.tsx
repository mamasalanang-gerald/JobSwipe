...
export default function SignUpSection() {
  ...
  return (
    <section className="relative min-h-screen bg-black flex items-center justify-center overflow-hidden py-10">
      {/* Background grid */}
      <div
      ...
      <div className="relative z-10 w-full max-w-sm mx-4" style={cardStyle}>
        <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-8 shadow-2xl">
          ...
          <h1 className="text-3xl font-bold text-white mb-1">Start swiping today</h1>
          <p className="text-white/60 text-sm mb-5">Create your free account</p>
            ...
            {(['jobseeker', 'recruiter'] as Role[]).map(r => (
              <button
                ...
                className={`px-4 py-1.5 rounded-full text-xs font-medium border transition-all duration-200 ${
                  role === r
                    ? 'bg-[#FF4E6A]/20 border-[#FF4E6A]/50 text-[#FF7854]'
                    : 'bg-white/[0.06] border-white/20 text-white/40 hover:text-white/70 hover:bg-white/10'
                }`}
              >
          ...
          <p className="text-center text-white/50 text-sm mt-5">
            ...
            <Link href="/login" className="text-[#FF4E6A] hover:opacity-80 transition font-semibold">
              Sign in
            </Link>
          </p>
        </div>

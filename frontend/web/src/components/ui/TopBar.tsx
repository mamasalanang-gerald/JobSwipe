...
interface TopBarProps {
  ...
  subtitle?: string;
  accentColor?: string;
  extraSlot?: React.ReactNode; // e.g. job post selector for company
}
...
export default function TopBar({
  ...
  return (
    <div style={{ flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 32px', height: '64px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
      <div>
        <h1 style={{ color: 'white', fontWeight: 700, fontSize: '17px', margin: 0 }}>{title}</h1>
        {subtitle && <p style={{ color: 'rgba(255,255,255,0.3)', fontSize: '12px', margin: 0 }}>{subtitle}</p>}
      </div>
